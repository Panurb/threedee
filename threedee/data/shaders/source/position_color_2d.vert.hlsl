...
Output main(Input input)
{
    ...
    float3x3 transform = float3x3(
        ...
        instance.transform_row2.xyz
    );
    float3 transformed_position = mul(transform, float3(input.position, 1.0f));
    float w = transformed_position.z;
    output.position = mul(projection_matrix, float4(transformed_position.xy, 0.0f, w));
    return output;
}
